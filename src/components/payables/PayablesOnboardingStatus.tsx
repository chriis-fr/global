...
interface PayablesOnboardingStatusProps {
  className?: string;
}

const CACHE_KEY = 'payables_onboarding_status';
const CACHE_DURATION = 5 * 60 * 1000; // 5 minutes

function readCache(): { isCompleted: boolean | null; valid: boolean } {
...
export default function PayablesOnboardingStatus({ className = '' }: PayablesOnboardingStatusProps) {
  const router = useRouter();
  const [state, setState] = useState<{ isCompleted: boolean | null; loading: boolean }>(() => {
    const { isCompleted, valid } = readCache();
  ...
  useEffect(() => {
    const load = async (background: boolean) => {
      ...
      try {
        const result = await getOnboardingStatus('accountsPayable');
      ...
      <div className="flex items-start space-x-3 sm:space-x-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-yellow-200 mb-4 text-sm sm:text-base">
            Before you can create payables, you need to configure your business information and payment settings. 
            Click &quot;Manage Payables Settings&quot; above or &quot;Complete Setup&quot; below to get started.
          </p>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => router.push('/dashboard/services/payables/onboarding')}
